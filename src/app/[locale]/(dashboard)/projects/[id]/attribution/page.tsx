...
export default async function AttributionPage({ params }: AttributionPageProps) {
  ...
  const [lastClickResult, comparisonResult] = await Promise.all([
    getAttributionAnalysisAction(projectId, AttributionModel.LAST_CLICK, dateRange, locale),
  ...
  const t = {
    title: locale === "fr" ? "Attribution Multi-Touch" : "Multi-Touch Attribution",
    subtitle:
      locale === "fr"
    ...
    comparison: locale === "fr" ? "Comparaison des modèles" : "Model Comparison",
    totalConversions: locale === "fr" ? "Conversions totales" : "Total Conversions",
    totalRevenue: locale === "fr" ? "Revenu total" : "Total Revenue",
    avgTouchpoints: locale === "fr" ? "Points de contact moyens" : "Avg. Touchpoints",
    ...
    noData:
      locale === "fr" ? "Aucune donnée d'attribution disponible" : "No attribution data available",
    setupInstructions:
      locale === "fr"
        ...
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <p className="text-2xl font-bold">{lastClickData.totalConversions}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-2xl font-bold">${lastClickData.totalRevenue.toLocaleString()}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-2xl font-bold">{lastClickData.avgTouchpoints}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center gap-2">
                  ...
                  />
                  <p className="text-lg font-bold capitalize">{topChannel?.channel || "-"}</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Tabs defaultValue="last_click" className="space-y-4">
            ...
            {comparisonData?.models.map((modelData) => (
              <TabsContent key={modelData.model} value={modelData.model.toLowerCase()}>
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <Share2 className="h-5 w-5" />
                      {modelLabels[modelData.model][locale === "fr" ? "fr" : "en"]}
                    </CardTitle>
                    <CardDescription>
                      {modelDescriptions[modelData.model][locale === "fr" ? "fr" : "en"]}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                {locale === "fr" ? "Comparaison des canaux" : "Channel Comparison"}
              </CardTitle>
              <CardDescription>
                {locale === "fr"
                  ...
                  <div key={channel.channel} className="space-y-2">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {comparisonData?.models.map((modelData) => {
                        ...
                        return (
                          <div key={modelData.model} className="text-center p-2 bg-muted rounded">
                            <p className="text-xs text-muted-foreground">
                              {
                                modelLabels[modelData.model][locale === "fr" ? "fr" : "en"].split(
                                  " "
                                )[0]
                              }
                            </p>
                            <p className="font-medium">{channelData?.percentage || 0}%</p>
                          </div>
                        );
                      })}
                    </div>
          ...
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <div className="flex gap-3">
              <Link href={`/projects/${projectId}/goals`}>
                <Button>{locale === "fr" ? "Configurer les objectifs" : "Set up Goals"}</Button>
              </Link>
            </div>
          </CardContent>
        </Card>
